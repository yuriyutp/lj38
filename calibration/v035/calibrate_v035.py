...
def run_calibration(args: argparse.Namespace) -> None:
    ...
    protocol_path = base / "CALIBRATION_PROTOCOL_V035.json"
    order_path = base / "CALIBRATION_EXECUTION_ORDER_V035.json"
    contract_path = base / "CONTRACT_A_V035.md"
    manifest2_path = base / "INSTRUMENT_MANIFEST_V035.json"
    expected = {
        contract_path: "cf2fa8d201d6d560f3bf6fed51bdd2842390d594e7307804445c141b7f605580",
        instrument_path: "4f411d1b54440aa33ac0403fe1a7fb0655b951f59bc13f70c3dff81b70657429",
        protocol_path: "d626ca30a611cda144195e38000ff3473d5135c2bac3ae84d58c06cd806bb7d4",
        order_path: "cbaa26cf3bfbd5cfa2b5f149f3cbdfa62579a836d86ea916527d17843ea3953e",
        manifest2_path: "e34084d3bdc41e4f2c7807f5a5f497493e85ed037664dfda4078c5fbf276772a",
    }
    ...
    if seeds != list(range(100, 106)):
        raise CalibrationError("calibration seed set is not 100-105")
    c = protocol["constants"]
    grids = protocol["candidate_grids"]
    outdir = base / "calibration_v035"
    outdir.mkdir(parents=True, exist_ok=True)
    ledger = Ledger(I, outdir / "CALIBRATION_RAW_V035.jsonl",
                    outdir / "CALIBRATION_STORE_V035.json")
    P = I.LJCluster(38)
    ...
    def method(key: str, name: str, seed: int, kwargs: dict[str, Any]) -> dict[str, Any]:
        if seed not in seeds:
            raise CalibrationError("attempted non-calibration seed")
        fn = I.METHODS[name][0]
    ...
    contract_b = {
        "contract_b_id":"lj38-audit-v0.3.5-contractB",
        ...
        "provenance":{"contract_a_sha256":expected[contract_path],"instrument_sha256":expected[instrument_path],
                      "instrument_manifest_sha256":expected[manifest2_path],"calibration_protocol_sha256":expected[protocol_path],
                      "execution_order_sha256":expected[order_path]}
    }
    contract_b_path=outdir/"CONTRACT_B_V035.json"
    ...
    binding_path=outdir/"CALIBRATION_BINDING_V035.json"; I.atomic_write_json(binding_path,binding)
    ...
    summary_path=outdir/"CALIBRATION_SUMMARY_V035.json"; I.atomic_write_json(summary_path,summary)

    artifacts=[]
    for path in (contract_b_path,binding_path,summary_path,ledger.raw_path,ledger.store_path,
                 Path(__file__).resolve(),protocol_path,order_path,manifest2_path):
    ...
    manifest_path=base/"MANIFEST_B_V035.json"
